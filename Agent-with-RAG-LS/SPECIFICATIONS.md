...
- The skills should have a separate skill vector database and be built as follows:
  - Each skill directory must contain a `SKILL.md` file formatted with YAML frontmatter (`name`, `description`).
  - The vectorizer embeds the complete `SKILL.md` file text (including metadata, YAML frontmatter, description, and trigger queries) to maximize semantic retrieval.
  - The vectors and the complete text of the `SKILL.md` file are stored in `skills_db` as one record per skill.
  - Upon start up, the app scans the `skills/` folder and loads new skills not currently in the skills vector database.
  - When the skill vector database is queried, matching skills meeting the threshold (or keyword heuristic fallbacks) are returned.
...
## Sample Skills and Tools
  - Get the time and weather of the city from the site that doesn't require API key. Calculate the target city's exact local timezone (e.g., Asia/Tokyo / JST) and return formatted `current_local_time` to ensure exact timestamps are presented without bracketed placeholders.
